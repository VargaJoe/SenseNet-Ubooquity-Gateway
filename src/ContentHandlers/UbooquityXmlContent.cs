...
{
    [ContentHandler]
    public class UbooquityXmlContent : SenseNet.ContentRepository.File, SenseNet.ContentRepository.IFile, IHttpHandler//, IWorkflowShoudWorkOnDis
    {
        public UbooquityXmlContent(Node parent) : this(parent, null) { }
        public UbooquityXmlContent(Node parent, string nodeTypeName) : base(parent, nodeTypeName) { }
        protected UbooquityXmlContent(NodeToken nt) : base(nt) { }
        ...
        private string[] refreshWithAction = { null, "Browse" };
        private bool IsRefreshTime
        {
            get
            {
                bool result = false;
                if (refreshWithAction.Any(s => s == SenseNet.Portal.Virtualization.PortalContext.Current.ActionName) && !string.IsNullOrWhiteSpace(this.CustomUrl))
                {
        ...
        public bool RefreshDocument()
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(this.CustomUrl))
                {
                    ...
                    feedRequest.Timeout = Timeout ?? 3000; // should be settings (milliseconds)
        ...
        private void SaveAsTechnicalUser()
        {
            ...
            try
            {
                ...
                using (new SystemAccount())
                {
                    ...
                    //var nodeToSave = this;
                    var nodeToSave = Node.Load<UbooquityXmlContent>(this.Id);
                    while (!ok && count > 0)
                    {
                        try
                        {
                            ...
                            {
                                nodeToSave = Node.Load<UbooquityXmlContent>(this.Id);
